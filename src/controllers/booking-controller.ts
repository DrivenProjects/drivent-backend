...
export async function getAllBookings(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const roomsBooked = await bookingService.getAllBookings();
    return res.status(httpStatus.OK).send(roomsBooked);
  } catch (error) {
    next(error);
  }
}
